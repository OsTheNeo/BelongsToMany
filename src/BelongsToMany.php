...
class BelongsToMany extends Field {
    ...
    public function __construct($name, $attribute = null, $resource = null) {
        ...
        $this->syncModel($resource, $attribute);
        $this->localize();
    }
    ...
    private function syncModel($resource, $attribute) {
        $this->fillUsing(function (
            ...
            if (is_subclass_of($model, 'Illuminate\Database\Eloquent\Model')) {
                $model::saved(function ($model) use ($attribute, $request) {
                    $inputValues = json_decode($request->input($attribute),
                        true);
                    if ($inputValues !== null) {
                        $values = array_column($inputValues, 'id');
                    } else {
                        $values = [];
                    }
                    if ( ! empty($this->pivot())) {
    ...
    public function dependsOn($dependsOnField, $tableKey) {
        return $this->withMeta([
            'dependsOn' => $dependsOnField, 'dependsOnKey' => $tableKey,
        ]);
    }

    public function rules($rules) {
        $rules = ($rules instanceof Rule || is_string($rules)) ? func_get_args()
            : $rules;
    ...
    public function jsonSerialize(): array {
        ...
        return array_merge([
            ...
            'indexName'                => $this->name, 'name' => $this->name,
