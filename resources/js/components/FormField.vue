	...
	methods: {
		...
		fetchOptions() {
			if (this.field.options) {
				...
				this.loading = true;
				let url = this.constructBaseUrl();
				this.sendNovaRequest(url);
			}
		},
		
		constructBaseUrl() {
			let baseUrl = `/nova-vendor/belongs_to_many/${this.resourceName}/options/${this.field.attribute}/${this.optionsLabel}`;
			
			if (this.isDependant && this.dependsOnValue) {
		...
		sendNovaRequest(url) {
			Nova.request(url).then((response) => {
